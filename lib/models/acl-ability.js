...
module.exports = function (Ability) {
	...
	function _addStaticActions(model) {
		...
		const security = _.get(modelClass, 'settings.security')
			|| _.get(modelClass, 'settings.sacl');

		if (security && security.acls) {
			const actions = _(security.acls).map(item => item.action).filter(item => !!item).uniq().value();
			return Ability.addActions(modelClass.modelName, actions);
		}
		return Promise.resolve();
	}
