...
module.exports = function (models) {
	...
	class ACL {
		...
		constructor(options) {
			options = options || {};
			Object.assign(this, models, _.pick(options, ['Role', 'Mapping', 'Ability', 'Permission']));
		...
		allowedResources(user, actions, resourceType) {
			...
			return this._allUserRoles(user).then(roles => this._allowedResources([user].concat(roles), actions, resourceType))
		...
		disallowedResources(user, actions, resourceType) {
			...
			return this._allUserRoles(user).then(roles => this._disallowedResources([user].concat(roles), actions, resourceType))
				.then(permissions => _.unionBy(permissions, p => (p.type || '_') + (p.id || '_')));
		 ...
		 * @private
		 */
		_disallowedResources(subjects, actions, resourceType) {
			subjects = utils.sureArray(subjects);
			if (!subjects.length) return Promise.resolve([]);
