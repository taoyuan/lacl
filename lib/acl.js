...
class ACL {
	...
	constructor(ds, options) {
		...
		const models = this.models = schema.loadModels(ds);
		_.forEach(models, (model, name) => models[_.lowerFirst(name)] = model);
	...
	resolveRoles(roles) {
		...
		return this.Role.getRolesParents(roles).then(parents => {
	...
	removePermissionsByResource(resource) {
		contract(arguments)
	...
	/**
	 * Remove permissions by subjects
	 *
	 * @param {Object|Array|String} subjects
	 */
	removePermissionsBySubjects(subjects) {
		contract(arguments)
		...
		_.forEach(subjects, subject => {
			const typeid = utils.typeid(subject);
			if (!typeid) return;
			typeids[typeid.type] = typeids[typeid.type] || [];
			typeids[typeid.type].push(typeid.id);
		});

		let where = _.map(typeids, (ids, type) => ({subjectType: type, subjectId: {inq: ids}}));
		where = where.length > 1 ? {and: where} : where[0];

		if (!where) throw new Error(g.f('Invalid subjects %j', subjects));
	...
	_recurseResourcePermissions(roles, resource) {
		...
		return this._resourcePermissions(roles, resource).then(actions => {
			return Role.getRolesParents(roles).then(parents => {
